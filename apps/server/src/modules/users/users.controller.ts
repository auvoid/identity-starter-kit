...
import { IsAuthenticated } from '../../middlewares/guards/auth.guard';
...
import { NotificationPreferences } from 'src/entities/notification-preference';
...
export class UsersController {
    ...
    async getCurrentUser(@CurrentUser() user: User) {
        ...
        const unread = await this.userNotificationsService.getUnreadCount(
            user.id,
        );
        return {
            ...user,
    ...
    async notificationPreferences(
        ...
        if (_preferences) {
            preferences =
                await this.notificationPreferencesService.findByIdAndUpdate(
                    _preferences.id,
                    body,
                );
        } else {
            preferences =
                await this.notificationPreferencesService.create(body);
    ...
    async verifyEmail(@CurrentUser() user: User) {
        ...
        const isAlreadyVerified = (await this.getCurrentUser(user))
            .emailVerified;

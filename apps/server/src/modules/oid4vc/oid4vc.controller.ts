...
import { StepActionsService } from '../applications/stepActions.service';
import {
    SiopOfferDTO,
    TokenResponseDTO,
    TokenRequestDTO,
...
export class Oid4vcController {
    ...
    async sendCredential(@Req() req: Request) {
        ...
        const approval = Math.floor(
            application.approvalTimeStamp.getTime() / 1000,
        );
        ...
        if (application.template.type === 'badge') {
            ...
            const response =
                await identity.issuer.createSendCredentialsResponse({
            ...
            const verifiableCredential =
                await identity.account.credentials.create({
                    recipientDid: did,
                    extras: {
                        credentialStatus: {
                            id: url + `#${applicationIndex}`,

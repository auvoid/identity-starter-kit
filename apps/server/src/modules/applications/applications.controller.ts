...
import { ApiOkResponsePaginated } from '../../decorators/ApiPaginatedResponse';
...
export class ApplicationsController {
    ...
    async createNewApplication(
        ...
        await this.notificationsService.applicationCreatedNotification(
    ...
    async createNewExternalApplication(@Body() body: CreateApplicationDTO) {
        ...
        if (!email || !validateEmail(email))
            throw new BadRequestException(
                errors.applications.BAD_RECIPIENT_EMAIL,
            );
        if (!templateId)
            throw new BadRequestException(
                errors.applications.MISSING_TEMPLATE_ID,
            );
        const template = await this.templatesService.findById(templateId, {
            organization: true,
        });
        if (!template)
            throw new BadRequestException(
                errors.applications.TEMPLATE_NOT_FOUND,
            );
        const customFields = template.customFields
            ? template.customFields.map((f) => f.fieldName).sort()
            : [];
        const bodyKeys = Object.keys(body.body).sort();
        if (!compareSorted(customFields, bodyKeys))
            throw new BadRequestException(
                errors.applications.BAD_APPLICATION_FORMAT,
            );
    ...
    async getAuthenticatedUserDetails(
        ...
        if (payload.scope !== 'flow-user-access-token')
            throw new ForbiddenException();
        const application = await this.applicationsService.findById(
            payload.applicationId,
            { stepActions: { stepConfig: true } },
        );
        const did = application.stepActions.find((f) => f.metadata.did).metadata
            .did;
        return { did };
    }
    ...
    async revokeCredential(@Param('id') id: string) {
        ...
        const applicationIndex =
            application.credentialIssuance.applicationIndex;
    ...
    async getApplicationByToken(@Body() body: ApplicationByTokenDTO) {
        ...
        if (!payload)
            throw new BadRequestException(errors.applications.NO_DATA);
        const application = await this.applicationsService.findById(
            payload.applicationId,
            {
                template: true,
    ...
    async getFlowApplication(
        ...
        if (currentStepConfig.type === 'presentation') {
            ...
            ['userForm', 'issuerForm'].includes(currentStepConfig.type)
        ) {
